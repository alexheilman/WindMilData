...
def main():
    ...
    if output_file == 'null' or attribute_file == 'null':
        ...
        negative_count, negative_names, sub_118_names, large_tap_names = NegPhaseCurrents(attribute_data)
        if negative_count == 0:
            ...
            np.savetxt("Files/OUTPUT WARNING - NEGATIVE CURRENT.txt", negative_names, fmt='%s', delimiter='\t')

        ## SAVE SUB 118 VOLTAGE BOXES AND PLUS 50A TAPS
        np.savetxt("Files/OUTPUT WARNING - LOW VOLTAGE BOXES.txt", sub_118_names, fmt='%s', delimiter='\t')
        np.savetxt("Files/OUTPUT WARNING - LARGE TAPS.txt", large_tap_names, fmt='%s', delimiter='\t')
...
def SyncData(output_data, attribute_data):
    ...
    # Citizens
    outcol_name     = 0 #column 1
    outcol_volt     = 2 #column 3
    outcol_edrop    = 4 #column 5 - existing drop (line 2) populated with 'Drop'
    outcol_miles    = 5 #column 6 
    outcol_ia       = 6 #column 7
    outcol_ib       = 7 #column 8
    outcol_ic       = 8 #column 9
    outcol_pdrop    = 3 #column 4 - proposed drop (line 3) populated with accumulated drop
    outcol_min      = 9 #column 10
    outcol_max      = 10 #column 11
...
def NegPhaseCurrents(attribute_data):
    ...
    for i in range(1, len(attribute_data)):
        ...
        ## TEMP - look for sub 118
        if attribute_data[i, attcol_edrop] != '<>' and attribute_data[i, attcol_edrop] != Mismatch_Val:
            if float(attribute_data[i, attcol_edrop]) >= 8.0:
                sub_118_names.append(attribute_data[i, attcol_name]) 

        ## TEMP - look for > 50A taps
        if attribute_data[i, attcol_i1] != '<>' and attribute_data[i, attcol_i1] != '' and attribute_data[i, attcol_i2] == '':
            temp_str = attribute_data[i, attcol_i1]
            temp_str = temp_str[1:]
            if float(temp_str) >= 50.0:
                large_tap_names.append(attribute_data[i, attcol_name]) 

    return negative_count, negative_names, sub_118_names, large_tap_names
