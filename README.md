...
The program is buttoned up into an executable file for use by the drafting staff.  The drafter copies and pastes the .exe file into the directory of the exported data files.  Once launched, a simple GUI prompts the user in file explorer to point to each of the export files, originating the file explorer in the current directory:
...
Different element information is contained within each file, so both exports were necessary for the program to function. 
